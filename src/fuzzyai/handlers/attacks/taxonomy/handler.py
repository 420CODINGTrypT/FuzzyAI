...
class TaxonomyHandler(BaseAttackTechniqueHandler):
    
    async def _attack(self, prompt: str, **extra: Any) -> AttackResultEntry:
        logger.debug(f"Generating taxonomy attack for prompt: {prompt}")
        
        llm: BaseLLMProvider = self._llms[0]
        
        taxonomy_prompt = safe_format(TAXONOMY_PROMPT, PROMPT=prompt)
        
        response = ""
        async for res in llm.generate(taxonomy_prompt, **extra):
            response = res.response
        
        return AttackResultEntry(original_prompt=prompt, current_prompt=taxonomy_prompt, response=response)
    
    @staticmethod
    def description() -> str:
        return "Taxonomy: Uses persuasive language techniques to jailbreak LLMs"
...
attack_handler_fm.register(FuzzerAttackMode.TAXONOMY, TaxonomyHandler)
